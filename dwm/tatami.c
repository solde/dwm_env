void 
tatami(Monitor *m) {
	static const unsigned int gaps = 6;        /* gap pixel between windows */
	...
	resize(c, nx+gaps, ny+gaps, (nw - 2 * c->bw) - gaps, (nh - 2 * c->bw) - gaps*2, False);
	...
	for(i = 0; c && (i < (tc % 5)); c = nexttiled(c->next))
	{
		...
		++i;
		resize(c, tnx+gaps, tny+gaps, (tnw - 2 * c->bw)-gaps*2, (tnh - 2 * c->bw)-gaps*2, False);
	}
